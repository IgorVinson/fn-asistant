...
import type { EvaluationContext, SlotStrategy } from "../types/evaluation.js";
import type { NormalizedJob, RequestedWindow } from "../types/job.js";
...
function shiftIsoByDays(referenceIso: string, daysToAdd: number): string {
...
function createFutureCounterWindow(
  requestedWindow: RequestedWindow,
  daysToAdd: number,
  workdayStart: string,
  workdayEnd: string
...
function isWeekend(anchorIso: string): boolean {
...
export async function evaluateJob(
...
): Promise<EvaluatedJobResult> {
  const busyIntervals = await calendarProvider.getBusyIntervalsForJob(job);
  const initialAvailability = evaluateAvailability(
    job.requestedWindow,
    policy.workingHours,
    busyIntervals
  );
  let decision = decideJob(job, policy, { busyIntervals });
  let slotStrategy: SlotStrategy = initialAvailability.requestedSlotAvailable
    ? "requested_slot"
  ...
  if (
    ...
    for (let dayOffset = 1; dayOffset <= policy.futureCounterSearchDays; dayOffset += 1) {
      const shiftedWindow = createFutureCounterWindow(
        job.requestedWindow,
        dayOffset,
      ...
      const futureBusyIntervals = await calendarProvider.getBusyIntervalsForDate(
        shiftedWindow.earliestStart
      );
      const futureAvailability = evaluateAvailability(
        shiftedWindow,
        policy.workingHours,
        futureBusyIntervals
      );
      ...
      if (
        futureAvailability.requestedSlotAvailable ||
        futureAvailability.hasSameDayCounterSlot
      ) {
        const counterStart =
          futureAvailability.feasibleRequestedStart ??
          futureAvailability.counterStart;
        const counterEnd =
          futureAvailability.feasibleRequestedEnd ??
