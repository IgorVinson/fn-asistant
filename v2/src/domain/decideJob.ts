...
export function decideJob(
  ...
  const isGranite = isGraniteJob(job);
  trace.push({
    rule: "granite_only",
  ...
  const rateTooLow =
    job.compensation.payType === "hourly" && hourlyRate > 0 && hourlyRate < baseRate;
  trace.push({
    rule: "base_rate",
    matched: rateTooLow,
  ...
  const travelExpense = calculateTravelExpense(
    job.distanceMiles,
    policy.travel
  );
  ...
  if (needsDateCounter || needsRateCounter || needsTravelCounter) {
    const decisionType =
      needsDateCounter && (needsRateCounter || needsTravelCounter)
        ? "counter_both"
        : needsDateCounter
          ? "counter_date"
          : "counter_rate";
