import type { WorkingHoursPolicy } from "../config/policy.js";
import type { BusyInterval, AvailabilityResult } from "../types/availability.js";
import type { RequestedWindow } from "../types/job.js";
...
function dateToMinutes(value: string): number {
  const date = new Date(value);
  return date.getHours() * 60 + date.getMinutes();
}

function buildIsoWithOriginalOffset(referenceIso: string, minutes: number): string {
...
function findEarliestFeasibleStart(
  startMinutes: number,
  endMinutes: number,
...
export function evaluateAvailability(
  ...
  const blockedIntervals = normalizeBusyIntervals(
    busyIntervals,
  ...
  const requestedStartCandidate = findEarliestFeasibleStart(
  ...
  let feasibleRequestedStart: string | undefined;
  let feasibleRequestedEnd: string | undefined;

  if (requestedStartCandidate !== undefined) {
    feasibleRequestedStart = buildIsoWithOriginalOffset(
      requestedStart,
      requestedStartCandidate
    );
    feasibleRequestedEnd = buildIsoWithOriginalOffset(
      requestedStart,
      requestedStartCandidate + durationMinutes
    );
  }
  ...
  const counterStartCandidate = requestedSlotAvailable
    ...
    : findEarliestFeasibleStart(
        Math.max(requestedStartMinutes, workdayStartMinutes),
        latestAllowedStartInWorkday,
        durationMinutes,
        blockedIntervals
      );

  const hasSameDayCounterSlot = counterStartCandidate !== undefined;
  const hasFutureCounterSlot =
  ...
  return {
    ...
    counterStart:
      counterStartCandidate !== undefined
        ? buildIsoWithOriginalOffset(requestedStart, counterStartCandidate)
        : undefined,
    counterEnd:
      counterStartCandidate !== undefined
        ? buildIsoWithOriginalOffset(
            requestedStart,
            counterStartCandidate + durationMinutes
          )
        : undefined,
