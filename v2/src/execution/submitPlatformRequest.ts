...
export function isWorkMarketLoginResponse(responseBody: string): boolean {
  return (
    /login\?redirectTo=/i.test(responseBody) ||
    /please sign in/i.test(responseBody) ||
    /<title>\s*login\b/i.test(responseBody)
  );
}

export function hasWorkMarketAssignmentMarkers(responseBody: string): boolean {
  return (
    /assignment-details/i.test(responseBody) &&
    /<title>.*-\s*Work Market<\/title>/i.test(responseBody)
  );
}

export function hasWorkMarketErrorMarkers(responseBody: string): boolean {
  return (
    /please correct the errors below/i.test(responseBody) ||
    /alert-error/i.test(responseBody) ||
    /--error/i.test(responseBody)
  );
}

export function validateWorkMarketResponse(responseBody: string): void {
  if (isWorkMarketLoginResponse(responseBody)) {
    throw new Error(
      "WorkMarket submission returned a login page; session is not authenticated"
    );
  }
...
export function isFieldNationLoginResponse(responseBody: string): boolean {
  return (
    /sign in/i.test(responseBody) ||
    /<title>\s*login\b/i.test(responseBody) ||
    /password/i.test(responseBody)
  );
}

export function isFieldNationJsonContentType(contentType: string | null): boolean {
...
export function validateFieldNationResponse(input: {
  ...
  if (isFieldNationLoginResponse(input.responseBody)) {
    throw new Error(
      "FieldNation submission returned a login page; session is not authenticated"
    );
  }
...
function extractCsrfToken(cookies: string): string {
...
async function submitMaterializedRequest(input: {
  ...
  body: string;
  platform: PlatformRequestSpec["platform"];
...
export async function submitPlatformRequest(
  ...
  if (request.platform === "FieldNation") {
    ...
    const userId = Number(options?.userId ?? process.env.FIELD_NATION_USER_ID ?? 983643);
