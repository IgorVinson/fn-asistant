...
import type { LegacyReplayJob } from "../workmarket/normalizeReplayJob.js";
...
export function normalizeFieldNationReplayJob(
  job: LegacyReplayJob,
  sourceRef: {
    sourceId: string;
    link?: string;
  ...
  if (job.platform !== "FieldNation") {
    throw new Error(`Expected FieldNation job, received ${job.platform}`);
  }
