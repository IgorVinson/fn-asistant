...
function isLegacyPrototypeSchema(db) {
    if (!hasTable(db, "jobs")) {
        return false;
    }
    ...
    return requiredJobColumns.some(requiredColumn => !columns.some(column => column.name === requiredColumn));
}
function resetLegacyPrototypeSchema(db) {
    db.exec(`
