...
export function describeAdvanceCounterRejection(workOrder, counterStart, now = Date.now()) {
  const counterOrder = {
    ...workOrder,
    time: { ...workOrder.time, start: counterStart },
  };
  const date = counterStart.toLocaleString("en-US", {
    timeZone: "America/New_York",
    month: "short", day: "numeric", hour: "numeric", minute: "2-digit",
  });
  return [
    "📅 Requested schedule unavailable; a later date was considered.",
    `Alternative: ${date} (Eastern)`,
    `💸 Alternative-date minimum: $${getMinPayThreshold(counterOrder, now)} · listed maximum: $${Math.round(getWorkOrderTotalPay(workOrder))}`,
    "Rejected: pay does not meet the alternative-date minimum.",
  ].join("\n");
}
