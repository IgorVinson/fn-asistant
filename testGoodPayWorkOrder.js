...
async function testWorkOrderWithGoodPay() {
    ...
    try {
        console.log('Testing work order with GOOD PAY:', {
            ...
            time: `${new Date(sampleWorkOrder.time.start).toLocaleString()} - ${new Date(sampleWorkOrder.time.end).toLocaleString()}`
        });
        ...
        console.error('❌ Error during work order eligibility test:', error.message);
        throw error;
    }
}
