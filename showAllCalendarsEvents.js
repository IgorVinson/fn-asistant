...
async function showAllCalendarsEvents() {
  ...
  try {
      ...
      try {
        ...
        if (todayEvents.length > 0) {
          todayEvents.forEach((event, index) => {
            ...
            console.log(
              `      👁️ Status: ${event.status}, Transparency: ${
    ...
    console.log("📊 SUMMARY:\n");
    console.log(
      `📅 Total calendars checked: ${calendarList.data.items.length}`
    );
    ...
    if (allEvents.length > 0) {
      ...
      console.log(
        "1. Choose which calendars should be checked for work order conflicts"
      );
      console.log(
        "2. Update the app to use multiple calendars for availability checking"
      );
      console.log(
        "3. Configure calendar priorities (e.g., personal vs work events)"
      );
    } else {
      console.log("\n✅ No events scheduled for today across all calendars!");
      console.log(
        "💡 This means any work order time would be available (no conflicts)"
      );
    }
  } catch (error) {
