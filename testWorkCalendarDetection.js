...
async function testWorkCalendarDetection() {
    ...
    try {
        ...
        console.log(`⏰ Testing time: ${conflictStart.toLocaleTimeString()} - ${conflictEnd.toLocaleTimeString()}`);
        ...
        console.log(`📊 Result: ${conflict.hasConflict ? '❌ CONFLICT DETECTED' : '✅ NO CONFLICT'}`);
        ...
        if (conflict.hasConflict) {
            ...
            conflict.conflicts.forEach((conf, i) => {
                console.log(`   ${i + 1}. "${conf.eventSummary}" (${conf.eventStart} - ${conf.eventEnd})`);
        ...
        console.log(`⏰ Testing time: ${noConflictStart.toLocaleTimeString()} - ${noConflictEnd.toLocaleTimeString()}`);
        ...
        console.log(`📊 Result: ${noConflict.hasConflict ? '❌ CONFLICT DETECTED' : '✅ NO CONFLICT'}`);
        ...
        console.log(`📅 Events found for ${today.toLocaleDateString()}: ${events.length}`);
