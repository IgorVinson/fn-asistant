...
function extractVerificationCode(emailContent) {
  ...
  if (sixDigitMatches) {
    ...
    for (const code of sixDigitMatches) {
      ...
      if (
        !emailContent.includes(`${code}px`) && // Not CSS pixel values
 ...
 */
export async function waitForWMcode(
  auth,
  timeoutMs = 60000,
  intervalMs = 3000
) {
  ...
  while (Date.now() - startTime < timeoutMs) {
    try {
      ...
      if (messages && messages.length > 0) {
        console.log(
          `🔍 Found ${messages.length} recent WorkMarket email(s), checking for verification code...`
        );
      ...
      console.log(
        `⏳ No verification code found yet, waiting ${
          intervalMs / 1000
